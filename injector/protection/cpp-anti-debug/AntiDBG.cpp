...
void DBG_MSG(WORD dbg_code, const char* message)
{
    ...
    printf(xor("[MSG-0x%X]: %s\n"), dbg_code, message);
    MessageBoxA(NULL, message, xor ("Security measures [Failed]"), 0);
#endif
}
...
void adbg_BeingDebuggedPEB(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_BEINGEBUGGEDPEB, xor ("Caught by BeingDebugged PEB check!"));
...
void adbg_CheckRemoteDebuggerPresent(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_CHECKREMOTEDEBUGGERPRESENT, xor ("Caught by CheckRemoteDebuggerPresent!"));
...
void adbg_CheckWindowName(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_FINDWINDOW, xor("Caught by FindWindow (WindowName)!"));
        exit(DBG_FINDWINDOW);
    }
}
...
void adbg_ProcessFileName(void)
{
    ...
    if (!(Process32FirstW(processList, &processInformation)))
        printf(xor("[Warning] It is impossible to check process list."));
    else
    {
        do
        {
            for (const wchar_t *debugger : debuggersFilename)
            {
                ...
                if (_wcsicmp(debugger, processName) == 0) {
                    DBG_MSG(DBG_PROCESSFILENAME, xor("Caught by ProcessFileName!"));
...
void adbg_CheckWindowClassName(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_FINDWINDOW, xor("Caught by FindWindow (ClassName)!"));
        exit(DBG_FINDWINDOW);
    }
}
...
void adbg_IsDebuggerPresent(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_ISDEBUGGERPRESENT, xor("Caught by IsDebuggerPresent!"));
...
void adbg_NtGlobalFlagPEB(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_NTGLOBALFLAGPEB, xor("Caught by NtGlobalFlag PEB check!"));
...
void adbg_NtQueryInformationProcess(void)
{
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (pPeb)
        {
            ...
            {
                DBG_MSG(DBG_NTQUERYINFORMATIONPROCESS, xor("Caught by NtQueryInformationProcess (ProcessDebugPort)!"));
...
void adbg_RDTSC(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_RDTSC, xor("Caught by RDTSC!"));
        exit(DBG_RDTSC);
    }
}
...
void adbg_QueryPerformanceCounter(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_QUERYPERFORMANCECOUNTER, xor("Caught by QueryPerformanceCounter!"));
...
void adbg_GetTickCount(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_GETTICKCOUNT, xor("Caught by GetTickCount!"));
...
void adbg_HardwareDebugRegisters(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_HARDWAREDEBUGREGISTERS, xor ("Caught by a Hardware Debug Register Check!"));
...
void adbg_MovSS(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_MOVSS, xor ("Caught by a MOV SS Single Step Check!"));
        exit(DBG_MOVSS);
    }
}
...
void adbg_CloseHandleException(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_CLOSEHANDLEEXCEPTION, xor ("Caught by an CloseHandle exception!"));
...
void adbg_SingleStepException(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_SINGLESTEPEXCEPTION, xor("Caught by a Single Step Exception!"));
...
void adbg_Int3(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_INT3CC, xor ("Caught by a rogue INT 3!"));
        exit(DBG_INT3CC);
    }
}
...
void adbg_PrefixHop(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_PREFIXHOP, xor ("Caught by a Prefix Hop!"));
        exit(DBG_PREFIXHOP);
    }
}
...
void adbg_Int2D(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_NONE, xor ("Caught by a rogue INT 2D!"));
        exit(DBG_NONE);
    }
}
