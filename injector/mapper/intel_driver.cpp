...
std::wstring intel_driver::GetDriverPath() {
	...
	if (temp.empty()) {
		return std::wstring(_W(L""));
	}
	return temp + std::wstring(_W(L"\\")) + GetDriverNameW();
}

bool intel_driver::IsRunning() {
	const HANDLE file_handle = CreateFileW(_W(L"\\\\.\\Nal"), FILE_ANY_ACCESS, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
...
HANDLE intel_driver::Load() {
	...
	HANDLE result = CreateFileW(_W(L"\\\\.\\Nal"), GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	...
	ntoskrnlAddr = utils::GetKernelModuleAddress(std::string(_("ntoskrnl.exe")));
	if (ntoskrnlAddr == 0) {
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}
...
bool intel_driver::ClearWdFilterDriverList(HANDLE device_handle) {
	...
	auto WdFilter = utils::GetKernelModuleAddress(std::string(_("WdFilter.sys")));
	if (!WdFilter) {
		return true;
	}

	auto RuntimeDriversList = FindPatternInSectionAtKernel(device_handle, _("PAGE"), WdFilter, (PUCHAR)"\x48\x8B\x0D\x00\x00\x00\x00\xFF\x05", _("xxx????xx"));
	if (!RuntimeDriversList) {
		return false;
	}

	auto RuntimeDriversCountRef = FindPatternInSectionAtKernel(device_handle, _("PAGE"), WdFilter, (PUCHAR)"\xFF\x05\x00\x00\x00\x00\x48\x39\x11", _("xx????xxx"));
	...
	*/
	auto MpFreeDriverInfoExRef = FindPatternInSectionAtKernel(device_handle, _("PAGE"), WdFilter, (PUCHAR)"\x49\x8B\xC9\x00\x89\x00\x08\xE8\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\xE9", _("xxx?x?xx???????????x"));
	if (!MpFreeDriverInfoExRef) {
		...
		*/
		MpFreeDriverInfoExRef = FindPatternInSectionAtKernel(device_handle, _("PAGE"), WdFilter, (PUCHAR)"\x48\x89\x4A\x00\x49\x8b\x00\xE8\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\xE9", _("xxx?xx?x???????????x"));
		if (!MpFreeDriverInfoExRef) {
			return false;
		}
		else {
...
uint64_t intel_driver::MmAllocateIndependentPagesEx(HANDLE device_handle, uint32_t size)
{
	...
	{
		kernel_MmAllocateIndependentPagesEx = intel_driver::FindPatternInSectionAtKernel(device_handle, (char*)_("PAGELK"), intel_driver::ntoskrnlAddr,
			...
			(char*)_("x????xxxxxxxx????xxxxxxxxx????xxxxxxxx"));
		if (!kernel_MmAllocateIndependentPagesEx) {
			return 0;
		}
...
bool intel_driver::MmFreeIndependentPages(HANDLE device_handle, uint64_t address, uint32_t size)
{
	...
	{
		kernel_MmFreeIndependentPages = intel_driver::FindPatternInSectionAtKernel(device_handle, _("PAGE"), intel_driver::ntoskrnlAddr,
			(BYTE*)"\xBA\x00\x60\x00\x00\x48\x8B\xCB\xE8\x00\x00\x00\x00\x48\x8D\x8B\x00\xF0\xFF\xFF", 
			(char*)_("xxxxxxxxx????xxxxxxx"));
		if (!kernel_MmFreeIndependentPages) {
			return false;
		}
...
BOOLEAN intel_driver::MmSetPageProtection(HANDLE device_handle, uint64_t address, uint32_t size, ULONG new_protect)
{
	...
	{
		kernel_MmSetPageProtection = intel_driver::FindPatternInSectionAtKernel(device_handle, _("PAGE"), intel_driver::ntoskrnlAddr,
			(BYTE*)"\x41\xB8\x00\x00\x00\x00\x48\x00\x00\x00\x8B\x00\xE8\x00\x00\x00\x00\x84\xC0\x74\x09\x48\x81\xEB\x00\x00\x00\x00\xEB", 
			(char*)_("xx????x???x?x????xxxxxxx????x"));
		if (!kernel_MmSetPageProtection) {
			return FALSE;
		}
...
{
	static uint64_t kernel_MmAllocatePagesForMdl = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, std::string(_("MmAllocatePagesForMdl")));
...
{
	static uint64_t kernel_MmMapLockedPagesSpecifyCache = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, std::string(_("MmMapLockedPagesSpecifyCache")));
...
{
	static uint64_t kernel_MmProtectMdlSystemAddress = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, std::string(_("MmProtectMdlSystemAddress")));
...
{
	static uint64_t kernel_MmUnmapLockedPages = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, std::string(_("MmUnmapLockedPages")));
...
{
	static uint64_t kernel_MmFreePagesFromMdl = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, std::string(_("MmFreePagesFromMdl")));
...
uint64_t intel_driver::AllocatePool(HANDLE device_handle, nt::POOL_TYPE pool_type, uint64_t size) {
	...
	static uint64_t kernel_ExAllocatePool = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, std::string(_("ExAllocatePoolWithTag")));
...
bool intel_driver::FreePool(HANDLE device_handle, uint64_t address) {
	...
	static uint64_t kernel_ExFreePool = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, std::string(_("ExFreePool")));
...
bool intel_driver::ExAcquireResourceExclusiveLite(HANDLE device_handle, PVOID Resource, BOOLEAN wait) {
	...
	static uint64_t kernel_ExAcquireResourceExclusiveLite = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, std::string(_("ExAcquireResourceExclusiveLite")));
...
bool intel_driver::ExReleaseResourceLite(HANDLE device_handle, PVOID Resource) {
	...
	static uint64_t kernel_ExReleaseResourceLite = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, std::string(_("ExReleaseResourceLite")));
...
BOOLEAN intel_driver::RtlDeleteElementGenericTableAvl(HANDLE device_handle, PVOID Table, PVOID Buffer) {
	...
	static uint64_t kernel_RtlDeleteElementGenericTableAvl = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, std::string(_("RtlDeleteElementGenericTableAvl")));
...
PVOID intel_driver::RtlLookupElementGenericTableAvl(HANDLE device_handle, PRTL_AVL_TABLE Table, PVOID Buffer) {
	...
	static uint64_t kernel_RtlDeleteElementGenericTableAvl = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, std::string(_("RtlLookupElementGenericTableAvl")));
